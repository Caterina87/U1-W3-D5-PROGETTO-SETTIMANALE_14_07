...
/* ESERCIZIO B
  Crea una variabile chiamata "random" e assegnaci un numero casuale tra 0 e 20 (deve essere generato dinamicamente a 
    ogni esecuzione).
*/
...
*/
const whoIsBigger = function (n, m) {
  if (n > m) {
    return n;
  } else {
    return m;
...
console.log("Esercizio 2", whoIsBigger(35, 87));
...
console.log("Esercizio 3", splitMe("Ciao, come stai?"));

/* ESERCIZIO 4
  Crea una funzione chiamata "deleteOne" che riceve una stringa e un booleano come parametri.
  Se il valore booleano è true la funzione deve ritornare la stringa senza il primo carattere, altrimenti la deve 
  ritornare senza l'ultimo.
*/
const deleteOne = function (myString, myBoolean) {
  if (myBoolean) {
...
console.log("Esercizio 4", deleteOne("Benvenuto", false));
...
const onlyLetters = function (stringWithNumber) {
  return stringWithNumber.replace(/[0-9]/g, "");
};
console.log("Esercizio 5", onlyLetters("Sono nata nel 1987 e adesso ho 35 anni"));

/* ESERCIZIO 6
  Crea una funzione chiamata "isThisAnEmail" che riceve una stringa come parametro e ritorna true se la stringa è un 
...
const rollTheDices = function (n) {
  ...
  for (let index = 0; index < n; index++) {
    let random = dice();
    ritorna.sum = ritorna.sum + random;
    ritorna.values.push(random);
  }
  return ritorna;
};
...
const howManyDays = function (myDate) {
  const diffInMs = new Date() - myDate;
  let diffInDays = diffInMs / (1000 * 60 * 60 * 24);
  diffInDays = parseInt(diffInDays);
  return diffInDays;
};
console.log("Esercizio 9", howManyDays(new Date(2023, 6, 4)));
...
const isTodayMyBirthday = function (myBirthday) {
  if (myBirthday.getMonth() === new Date().getMonth() && myBirthday.getDay() === new Date().getDay()) {
    return true;
  } else {
    return false;
  }
};
console.log("Esercizio 10", isTodayMyBirthday(new Date(1987, 8, 15)));
...
*/
/*const deleteProp = function (myString2, myObject) {
  for (let index = 0; index < myObject.length; index++) {
    delete myObject[index].myString2;
  }
  return myObject;
};
console.log("Esercizio 11", deleteProp());*/
...
const onlyTheYears = function (movies) {
  const soloAnni = [];
  for (let index = 0; index < movies.length; index++) {
    soloAnni.push(movies[index].Year);
  }
  return soloAnni;
};
...
*/
const searchByTitle = function (movies, titleStr) {
  let onlyTitle = [];
  for (let index = 0; index < movies.length; index++) {
    if (movies[index].Title === titleStr) {
      onlyTitle.push(movies[index]);
    }
  }
  return onlyTitle;
};
console.log("Esercizio 17", searchByTitle(movies, "Lord of War"));
...
*/
const testoTd = function () {
  const arrayTd = document.getElementsByTagName("td");

  for (let index = 0; index < arrayTd.length; index++) {
    console.log("testoTd", arrayTd[index].innerText);
  }
  return arrayTd;
};
console.log("Esercizio 22", testoTd());
...
const sfondoRosso = function () {
  const arrayDiA = document.getElementsByTagName("a");

  for (let index = 0; index < arrayDiA.length; index++) {
    arrayDiA[index].classList.add(".backgroundRed");
  }
};
...
const addElement = function () {
  const list = document.getElementsById("myList");
...
/* ESERCIZIO 29
  Crea una funzione chiamata "isItPrime" che riceve un numero come parametro e ritorna true se il numero fornito è 
  un numero primo.
*/
